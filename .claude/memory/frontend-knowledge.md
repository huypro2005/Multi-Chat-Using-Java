...
`src/components/ProtectedRoute.tsx` — wrap routes cần auth.
Đọc `accessToken` từ `useAuthStore`, tính `isAuthenticated = !!accessToken`. Nếu false → Navigate to /login.
Dùng từ Tuần 3+ cho /conversations, /profile.
Lưu ý: authStore không có `isAuthenticated` property trực tiếp — phải compute từ `accessToken`.
