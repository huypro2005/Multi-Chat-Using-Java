...
## TODO Tuần 2 — Migrate globalThis workaround sang tokenStorage.ts

**Tại sao:** Hiện tại `api.ts` đọc token qua `globalThis.__authStoreGetState` để tránh circular dep với `authStore.ts`. Khi Tuần 2 implement `login`/`logout` action trong store (cần gọi `api.post`), vòng circular sẽ trở thành thực.

**Pattern thay thế cần implement ở Tuần 2:**

Tạo `src/lib/tokenStorage.ts`:
- Module này KHÔNG import `api.ts` (phá circular dep)
- Export: `register(fns)`, `getAccessToken()`, `getRefreshToken()`, `setAuth()`, `clearAuth()`
- `authStore.ts` gọi `tokenStorage.register(...)` khi khởi tạo
- `api.ts` import `tokenStorage.ts` (không import authStore)
- Kết quả: type-safe, testable, không cần `any`, không cần `globalThis`

**Ưu tiên:** Bắt buộc trước khi implement login/logout action trong authStore (Tuần 2, Phase 1).
