...
_Last updated: 2026-04-21 (W7-D1 contract + ADR-020: W7-1/2/3 added to pre-production; auto-transfer race, empty group, max-50 enforcement)_
...
| W7-1 | **Auto-transfer race khi OWNER leave** — `POST /api/conversations/{id}/leave` khi caller là OWNER: service SELECT ADMIN/MEMBER oldest `joined_at` → promote thành OWNER + DELETE row của caller. Nếu 2 request concurrent (OWNER leave đồng thời với ADMIN leave hoặc kick) → có thể 2 transaction cùng đọc "ADMIN oldest" → cùng promote cùng user, HOẶC promote user vừa bị kick. Mitigation bắt buộc: `SELECT ... FOR UPDATE` trên rows `conversation_members WHERE conversation_id = :id` trong cùng `@Transactional` — lock toàn bộ members rows trước khi quyết định promote. Acceptable performance vì group ≤50 members, lock ngắn. Test: concurrent leave + kick từ 2 client, verify final state consistent (exactly 1 OWNER, đúng user được promote). | `ConversationService.leaveGroup()` (sẽ implement W7-D2) | M | W7-D2 khi BE implement /leave |
...
| W7-3 | **Max 50 enforcement race** — 2 ADMINs concurrent gọi `POST /api/conversations/{id}/members` với 2 batch khác nhau: cả 2 SELECT COUNT = 48 → mỗi ADMIN add 3 → total 54 > 50. Mitigation bắt buộc: `SELECT COUNT(*) FROM conversation_members WHERE conversation_id = :id FOR UPDATE` lock trong transaction trước INSERT. Alternative: `INSERT ... WHERE (SELECT COUNT(*) ...) < 50 - array_length(:new_ids, 1)` conditional insert. Lock approach đơn giản hơn — giữ cho V1. Test: 2 concurrent add → 1 thành công full batch, 1 trả `GROUP_FULL`. | `ConversationService.addMembers()` (sẽ implement W7-D2) | S | W7-D2 khi BE implement add members |
